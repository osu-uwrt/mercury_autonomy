...
void SetFeedforwardMode::rosInit()
{
  ...
  set_teleop_client_ =
    rosNode()->create_client<SetBool>("setTeleop");
}
...
BT::NodeStatus SetFeedforwardMode::onStart()
{
  ...
  if (set_teleop_client_->service_is_ready()) {
    ...
    RCLCPP_DEBUG(
      rosNode()->get_logger(), "SetFeedforwardMode: setTeleop service not ready, skipping call");
  }
  ...
  RCLCPP_INFO(
    rosNode()->get_logger(),
    "SetFeedforwardMode: Published FEEDFORWARD mode to controller/linear and controller/angular");

  return BT::NodeStatus::SUCCESS;
}

BT::NodeStatus SetFeedforwardMode::onRunning()
{
  return BT::NodeStatus::SUCCESS;
}
