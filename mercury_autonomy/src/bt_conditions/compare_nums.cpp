...
BT::NodeStatus CompareNums::tick() {
    ...
    if (!test || !a || !b) {
        RCLCPP_ERROR(
            rosNode()->get_logger(), "CompareNums: missing required input (test=%s, a=%s, b=%s)",
    ...
    if (op == ">") {
        ...
        RCLCPP_ERROR(
            rosNode()->get_logger(),
            "CompareNums: invalid operator '%s'. Use >, <, ==, >=, <=, or !=", op.c_str());
        return BT::NodeStatus::FAILURE;
    }

    RCLCPP_DEBUG(
        rosNode()->get_logger(), "CompareNums: %.4f %s %.4f -> %s", a.value(), op.c_str(),
        b.value(), result ? "SUCCESS" : "FAILURE");
