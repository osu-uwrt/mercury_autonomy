...
public:
    TreeExecutor() : Node("tree_executor") {
        // Declare parameters
        ...
        for (const auto & plugin_path : extra_plugins_) {
            try {
                ...
                RCLCPP_ERROR(
                    get_logger(), "Failed to load plugin %s: %s", plugin_path.c_str(), e.what());
            }
        }

        // Action server: execute_tree (replaces the old topic-based trigger)
        action_server_ = rclcpp_action::create_server<ExecuteTree>(
            this, "autonomy/execute_tree", std::bind(&TreeExecutor::handleGoal, this, _1, _2),
        ...
        list_trees_srv_ = create_service<mercury_msgs::srv::ListTrees>(
            "autonomy/list_trees", std::bind(&TreeExecutor::handleListTrees, this, _1, _2));
        ...
        RCLCPP_INFO(
            get_logger(),
            "TreeExecutor ready (tick rate: %.1f Hz). "
...
private:
    ...
    rclcpp_action::GoalResponse handleGoal(
        const rclcpp_action::GoalUUID & /*uuid*/, std::shared_ptr<const ExecuteTree::Goal> goal) {
    ...
    void handleAccepted(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        exec_thread_ = std::thread([this, goal_handle]() { executeTree(goal_handle); });
    }
    ...
    void executeTree(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        try {
            ...
            RCLCPP_INFO(
                get_logger(), "Tree finished: %s (%.2fs)", BT::toStr(bt_status).c_str(), elapsed);
    ...
    void finishWithError(
        const std::shared_ptr<GoalHandle> & goal_handle, const std::string & error_msg) {
        MercuryBtNode::staticDeinit();
        tree_running_.store(false);
        cancel_requested_.store(false);
        publishStatus("ERROR");
